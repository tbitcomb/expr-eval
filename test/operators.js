...
describe('Operators', async function () {
  ...
  describe('cbrt(x)', async function () {
    it('returns the cube root of x', async function () {
      ...
      assert.strictEqual(await parser.parse('cbrt x').toJSFunction('x')(27), 3);
    });
  });
  ...
  describe('expm1(x)', async function () {
    it('returns e^x - 1', async function () {
      ...
      assertCloseTo(await parser.parse('expm1 x').toJSFunction('x')(1), 1.718281828459045, delta);
      assertCloseTo(await parser.parse('expm1 x').toJSFunction('x')(2), 6.38905609893065, delta);
    });
  });
  ...
  describe('log1p(x)', async function () {
    it('returns e^x - 1', async function () {
      ...
      assertCloseTo(await parser.parse('log1p x').toJSFunction('x')(1), 0.6931471805599453, delta);
      assertCloseTo(await parser.parse('log1p x').toJSFunction('x')(9), 2.302585092994046, delta);
    });
  });
  ...
  describe('log2(x)', async function () {
    it('returns the base 2 log of x', async function () {
      ...
      assert.strictEqual(await parser.parse('log2 x').toJSFunction('x')(4), 2);
      assertCloseTo(await parser.parse('log2 x').toJSFunction('x')(3), 1.584962500721156, delta);
    });
  });
});
